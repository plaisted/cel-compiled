...
{
    private readonly record struct CacheKey(Type ContextType, Type ResultType, CelExpr Expr, CelBinderMode BinderMode);
    ...
    {
        var key = new CacheKey(typeof(TContext), typeof(object), expr, options.BinderMode);
    ...
    {
        var key = new CacheKey(typeof(TContext), typeof(TResult), expr, options.BinderMode);
