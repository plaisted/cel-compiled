...
The library SHALL provide a public way to render source-aware diagnostics into line/column/snippet output suitable for logs, CLIs, or UI surfaces for parse, compile, checked-compilation, and attributed runtime failures.
...
#### Scenario: Formatted checked-compilation diagnostic highlights invalid selection
- **WHEN** a caller formats a source-aware checked-compilation failure caused by an invalid identifier, member selection, or index access
- **THEN** the rendered diagnostic identifies the specific failing subexpression span rather than only the full expression
