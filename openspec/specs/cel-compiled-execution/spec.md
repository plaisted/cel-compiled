...
The library MUST NOT automatically promote between `int`, `uint`, and `double` for arithmetic operators. Mixed-type arithmetic SHALL produce a `no_matching_overload` error per the CEL specification.

#### Scenario: Reject mixed-type arithmetic
- **WHEN** a compiled expression evaluates `1 + 1u` or `1 + 1.0`
- **THEN** the result is a `no_matching_overload` error

#### Scenario: Accept same-type arithmetic
- **WHEN** a compiled expression evaluates `1 + 2` (int+int) or `1u + 2u` (uint+uint)
...
- **WHEN** a compiled expression evaluates `int(1u)` or `double(1)` or `uint(1)`
- **THEN** the result is the correctly converted numeric value

#### Scenario: Reject out-of-range conversions
- **WHEN** a compiled expression evaluates `int(double_too_large)` or `uint(-1)`
